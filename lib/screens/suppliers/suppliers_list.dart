...
class _ProductsState extends State<Suppliers> {
  List<CompanyModel> products = [];
  final _suppliersStream = StreamController<List<CompanyModel>>.broadcast();
  ...
  PreferredSize _appBar(BuildContext context) {
    return appBar(context, 'Proveedores',
        elevation: false,
        radius: 0,
  ...
  Widget _body() {
    return Column(
      children: [_searchBar().paddingBottom(5), _suppliersList().expand()],
    );
  }
  ...
  Widget _suppliersList() {
    return Padding(
      ...
      child: FutureBuilder<List<Map>?>(
          future: CompaniesProvider.getAllSuppliers(limit: 50),
          builder: (BuildContext context, AsyncSnapshot<List<Map>?> snapshot) {
            if (snapshot.hasData) {
              return StreamBuilder<List<CompanyModel>?>(
                  stream: _suppliersStream.stream,
                  builder:
