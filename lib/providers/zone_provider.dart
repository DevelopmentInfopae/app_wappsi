...
class ZonesProvider {
  ...
  static Future<Map<String, dynamic>> getLocationData(
      {int? zoneId, int? subzoneId}) async {
    ZoneModel? zone;
    SubzoneModel? subzone;
    final res = await DataProvider().postPetition(locationDataEndP,
        {"zone_id": zoneId, "subzone_id": subzoneId}, dataBloc.getHeaders());
    if (!(res['error'] ?? true)) {
      try {
  ...
  static Future<Map<String, dynamic>> getZoneSzoneDataJson(
      {int? zoneId, int? subzoneId, int timeLimit = 5}) async {
    ZoneModel? zone;
    SubzoneModel? subzone;
    final res = await DataProvider().postPetition(locationDataEndP,
        {"zone_id": zoneId, "subzone_id": subzoneId}, dataBloc.getHeaders(),
